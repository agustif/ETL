...
const groupByKey = ({ data, config }: GroupByOptions) =>
  data.reduce((result: any, accumulator) => {
    result[accumulator[config.group]] = [...(result[accumulator[config.group]] || []), accumulator]
    return result
  }, {})

const orderByValue = ({ data, config }: GroupByOptions): Brewery[] => {
  const { order, orderBy } = config
  if (order === OrderOptions.Ascendent) data.sort((a, b) => (a[orderBy] < b[orderBy] && 1) || -1)
  else if (order === OrderOptions.Descendent) data.sort((a, b) => (a[orderBy] > b[orderBy] && 1) || -1)
  return data
}
